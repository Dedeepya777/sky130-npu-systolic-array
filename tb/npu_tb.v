...
// Target: Icarus Verilog, Verilator, Synopsys, Cadence, ModelSim, Vivado
//
...
module npu_tb;
    ...
    // ------------------------------------------------------------------------
    // DUT Signals (Flat 1D bitvectors for universal simulator/synthesis compatibility)
    // ------------------------------------------------------------------------
    reg                             clk;
    reg                             rst_n;
    reg                             clr;
    ...
    // ------------------------------------------------------------------------
    // Testbench Matrix Storage (Module-level for broad simulator compatibility)
    // ------------------------------------------------------------------------
    ...
    task run_test(
        ...
        begin
            ...
            // Sample Row 0 on the first valid cycle
            for (c = 0; c < N; c = c + 1) begin
            ...
            if (err_count == 0) begin
                $display(">>> [PASS] %0s: All %0d matrix elements match golden model perfectly!", test_title, N*N);
                passed_tests = passed_tests + 1;
            end else begin
                $display(">>> [FAIL] %0s: %0d mismatch(es) detected!", test_title, err_count);
